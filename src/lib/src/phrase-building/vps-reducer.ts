...
import {
  isKedulStat,
  isKawulStat,
} from "../parsing/verb-section/parse-verb-helpers";
...
export function vpsReducer(entryFeeder: T.EntryFeeder) {
  return function (
      ...
      if (action.type === "set externalComplement") {
        ...
        if (selection?.selection.type === "adjective" && kawulKedul !== false) {
          const matchingStatVerbs = (
            ...
            if (kawulKedul === "kedul") {
              return x.entry.c.includes("stat. comp. intrans.");
            } else {
              return x.entry.c.includes("stat. comp. trans.");
            }
          });
          if (matchingStatVerbs.length) {
            return makeVPSelectionState(matchingStatVerbs[0], {
              ...vps,
